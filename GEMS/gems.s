...
_gemsinit
	...
	moveq	 #-1,d0
	move.l	d0,-(sp)		; MDT: KC modification, A7 changed to SP
	jsr	_gemsputcbyte(pc)	; MDT: KC modification, (pc) added
	moveq	 #11,d0
	move.l	d0,-(sp)		; MDT: KC modification, A7 changed to SP
	jsr	_gemsputcbyte(pc)	; MDT: KC modification, (pc) added

	move.l	#_patchbank,-(sp)	; MDT: KC modification, changed to direct pointer, original line below
;	move.l	8(A6),-(A7)
	jsr	_gemsputptr(pc)		; MDT: KC modification, (pc) added
	move.l	#_envbank,-(sp)		; MDT: KC modification, changed to direct pointer, original line below
;	move.l	12(A6),-(A7)
	jsr	_gemsputptr(pc)		; MDT: KC modification, (pc) added
	move.l	#_seqbank,-(sp)		; MDT: KC modification, changed to direct pointer, original line below
;	move.l	16(A6),-(A7)
	jsr	_gemsputptr(pc)		; MDT: KC modification, (pc) added
	move.l	#_sampbank,-(sp)	; MDT: KC modification, changed to direct pointer, original line below
;	move.l	20(A6),-(A7)
	jsr	_gemsputptr(pc)		; MDT: KC modification, (pc) added
	unlk a6
